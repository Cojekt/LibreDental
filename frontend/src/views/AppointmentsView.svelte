<script lang="ts">
  ...
  const statusBadges: Record<string, { label: string; bg: string; text: string; border: string }> = $derived({
    scheduled: { label: m.appts_status_scheduled(), bg: "bg-blue-500/15", text: "text-blue-400", border: "border-blue-500/30" },
    confirmed: { label: m.appts_status_confirmed(), bg: "bg-sky-500/15", text: "text-sky-400", border: "border-sky-500/30" },
    arrived: { label: m.appts_status_arrived(), bg: "bg-amber-500/15", text: "text-amber-400", border: "border-amber-500/30" },
    in_chair: { label: m.appts_status_in_chair(), bg: "bg-purple-500/15", text: "text-purple-400", border: "border-purple-500/30" },
    completed: { label: m.appts_status_completed(), bg: "bg-emerald-500/15", text: "text-emerald-400", border: "border-emerald-500/30" },
    cancelled: { label: m.appts_status_cancelled(), bg: "bg-rose-500/15", text: "text-rose-400", border: "border-rose-500/30" },
    no_show: { label: m.appts_status_no_show(), bg: "bg-slate-500/15", text: "text-slate-400", border: "border-slate-500/30" },
  });
...
<div class="space-y-6">
  ...
  <div class="flex flex-wrap items-center justify-between gap-4 rounded-xl border border-slate-700/80 bg-slate-800/80 p-4 shadow-sm backdrop-blur">
    ...
    <div class="flex items-center gap-3">
      <div class="flex items-center gap-2">
        <label for="provider-filter" class="text-xs font-medium text-slate-400">{m.appts_provider_filter()}</label>
        <select
          id="provider-filter"
