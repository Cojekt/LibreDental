<script lang="ts">
  ...
  let scheduledCount = $derived(appointments.filter((a: Appointment) => a.status === "scheduled").length);
  let confirmedCount = $derived(appointments.filter((a: Appointment) => a.status === "confirmed").length);
  let arrivedCount = $derived(appointments.filter((a: Appointment) => a.status === "arrived").length);
  let inChairCount = $derived(appointments.filter((a: Appointment) => a.status === "in_chair").length);
  let completedCount = $derived(appointments.filter((a: Appointment) => a.status === "completed").length);
</script>
