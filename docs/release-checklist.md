...
> [!WARNING]
> Consolidating migrations into a single baseline is a destructive operation for existing databases. Replacing the migration chain with a fresh baseline that issues `CREATE TABLE` will fail on any installed database that has already applied earlier migrations. Do NOT perform baseline consolidation on live installs without a data-migration plan, an agreed version boundary, and testing on a real snapshot.
...
5. Run `go test ./...` to ensure tests pass with the new baseline.
