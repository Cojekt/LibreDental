...
func SeedDatabase(db *sqlite.DB) (*SeedSummary, error) {
	...
	for _, fs := range feeSchedules {
		if err := procedureRepo.Save(ctx, fs); err != nil {
			return nil, fmt.Errorf("failed to seed fee schedule %s: %w", fs.ID, err)
		}
	...
	for _, b := range bundles {
		if err := bundleRepo.Create(ctx, b); err != nil {
			return nil, fmt.Errorf("failed to seed treatment bundle %s: %w", b.ID, err)
		}
	...
	for _, c := range claims {
		if err := claimRepo.Create(ctx, c); err != nil {
			return nil, fmt.Errorf("failed to seed claim %s: %w", c.ID, err)
		}
		summary.ClaimsCount++
	}
	...
	for _, p := range payments {
		if err := paymentRepo.Create(ctx, p); err != nil {
			return nil, fmt.Errorf("failed to seed payment %s: %w", p.ID, err)
		}
